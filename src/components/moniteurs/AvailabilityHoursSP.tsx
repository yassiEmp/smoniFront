...
type AvailabilityHoursSPProps = {
  selectedHours: string[];
  setSelectedHours: (hours: string[]) => void;
  takenHours?: string[]; // heures déjà prises par d'autres combinaisons véhicule/lieu
};

const hours = Array.from({ length: 17 }, (_, i) => 6 + i);

export default function AvailabilityHoursSP({ selectedHours, setSelectedHours, takenHours = [] }: AvailabilityHoursSPProps) {
  const handleHourClick = (hour: number) => {
    const hourStr = `${String(hour).padStart(2, '0')}:00`;
    if (takenHours.includes(hourStr)) return; // Ne rien faire si l'heure est déjà prise
